...
async def recognize_order_image(
    ...
    prompt = f"""この画像は手書きの注文台帳です。以下の構造と注意事項をよく理解してから読み取ってください。

【注文台帳の構造】
- 表の左端の列：商品名（縦方向に商品が並ぶ）。これは顧客名ではありません
- 表の上部の各列：列の一番上のセルに顧客名が1人ずつ書かれています
- 顧客名の下のセル：その顧客の注文日（例: 4/17）
- 表の下部の「謝礼」行：各顧客列の注文金額合計
