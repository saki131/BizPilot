...
def set_api_keys(api_keys: list[str]):
    """後方互換性のために残しているが、最初のキーのみ使用する。"""
    if api_keys:
        configure(api_keys[0])
...
def generate_content_with_image(model_name: str, prompt: str, image_b64: str, max_retries: int = None):
    """Generate content using available GenAI SDK with a single API key.
    ...
    if HAS_GENAI_LEGACY and genai_legacy is not None:
        try:
            ...
            print("[DEBUG genai_wrapper] Successfully generated content")
            return response
        except Exception as e:
