...
def generate_sales_receipt_pdf(invoice: SalesInvoice, db: Session) -> BytesIO:
    ...
    font_name = setup_japanese_font()
    pdf.setFont(font_name, 24)
    pdf.drawCentredString(width / 2, height - 40*mm, "領　収　証")
    pdf.setFont(font_name, 12)
    pdf.drawString(30*mm, height - 60*mm, f"{sales_person.name if sales_person else ''}　様")
    amount_y = height - 85*mm
    pdf.setFont(font_name, 10)
    pdf.drawString(30*mm, amount_y, "下記の通り領収いたしました")
    amount_y -= 15*mm
    pdf.rect(30*mm, amount_y - 10*mm, 150*mm, 15*mm)
    pdf.setFont(font_name, 18)
    amount_str = f" {invoice.total_amount_inc_tax:,}"
    pdf.drawString(35*mm, amount_y - 7*mm, amount_str)
    tadashi_y = amount_y - 25*mm
    pdf.setFont(font_name, 10)
    note_text = invoice.note if invoice.note else "御品代として"
    pdf.drawString(30*mm, tadashi_y, f"但し　{note_text}")
    date_y = tadashi_y - 20*mm
    receipt_date = invoice.receipt_date if invoice.receipt_date else (invoice.invoice_date if invoice.invoice_date else datetime.now().date())
    pdf.drawString(30*mm, date_y, f"上記正に領収いたしました")
    pdf.drawString(30*mm, date_y - 10*mm, f"{receipt_date.year}年{receipt_date.month}月{receipt_date.day}日")
    issuer_y = date_y - 30*mm
    pdf.setFont(font_name, 10)
    pdf.drawString(120*mm, issuer_y, COMPANY_INFO['postal_code'])
    pdf.drawString(120*mm, issuer_y - 7*mm, COMPANY_INFO['address1'])
    pdf.drawString(120*mm, issuer_y - 14*mm, COMPANY_INFO['address2'])
    pdf.setFont(font_name, 12)
    pdf.drawString(120*mm, issuer_y - 24*mm, COMPANY_INFO['name'])
    pdf.setFont(font_name, 10)
    pdf.drawString(120*mm, issuer_y - 31*mm, f"代表　{COMPANY_INFO['representative']}")
    pdf.circle(165*mm, issuer_y - 24*mm, 8*mm, stroke=1, fill=0)
    pdf.save()
    buffer.seek(0)
    return buffer
...
def generate_contractor_receipt_pdf(invoice: ContractorInvoice, db: Session) -> BytesIO:
    "`Contractor Receipt PDF generation"`
...
def generate_sales_receipt_pdf(invoice: SalesInvoice, db: Session) -> BytesIO:
    ...
    font_name = setup_japanese_font()
    pdf.setFont(font_name, 24)
    pdf.drawCentredString(width / 2, height - 40*mm, "鬆倥蜿弱險ｼ")
    pdf.setFont(font_name, 12)
    pdf.drawString(30*mm, height - 60*mm, f"{sales_person.name if sales_person else ''}縲讒・)
    amount_y = height - 85*mm
    pdf.setFont(font_name, 10)
    pdf.drawString(30*mm, amount_y, "荳玖ｨ倥・騾壹ｊ鬆伜庶縺・◆縺励∪縺励◆")
    amount_y -= 15*mm
    pdf.rect(30*mm, amount_y - 10*mm, 150*mm, 15*mm)
    pdf.setFont(font_name, 18)
    amount_str = f"ﾂ･ {invoice.total_amount_inc_tax:,}"
    pdf.drawString(35*mm, amount_y - 7*mm, amount_str)
    tadashi_y = amount_y - 25*mm
    pdf.setFont(font_name, 10)
    note_text = invoice.note if invoice.note else "蠕｡蜩∽ｻ｣縺ｨ縺励※"
    pdf.drawString(30*mm, tadashi_y, f"菴・＠縲{note_text}")
    date_y = tadashi_y - 20*mm
    receipt_date = invoice.receipt_date if invoice.receipt_date else (invoice.invoice_date if invoice.invoice_date else datetime.now().date())
    pdf.drawString(30*mm, date_y, f"荳願ｨ俶ｭ｣縺ｫ鬆伜庶縺・◆縺励∪縺励◆")
    pdf.drawString(30*mm, date_y - 10*mm, f"{receipt_date.year}蟷ｴ{receipt_date.month}譛・receipt_date.day}譌･")
    issuer_y = date_y - 30*mm
    pdf.setFont(font_name, 10)
    pdf.drawString(120*mm, issuer_y, COMPANY_INFO['postal_code'])
    pdf.drawString(120*mm, issuer_y - 7*mm, COMPANY_INFO['address1'])
    ...
    pdf.drawString(120*mm, issuer_y - 24*mm, COMPANY_INFO['name'])
    pdf.setFont(font_name, 10)
    pdf.drawString(120*mm, issuer_y - 31*mm, f"莉｣陦ｨ縲{COMPANY_INFO['representative']}")
    pdf.circle(165*mm, issuer_y - 24*mm, 8*mm, stroke=1, fill=0)
    pdf.save()
    buffer.seek(0)
    return buffer
...
def generate_contractor_receipt_pdf(invoice: ContractorInvoice, db: Session) -> BytesIO:
    ...
    pdf.drawCentredString(width / 2, height - 40*mm, "鬆倥蜿弱險ｼ")
    pdf.setFont(font_name, 12)
    pdf.drawString(30*mm, height - 60*mm, f"{contractor.name if contractor else ''}縲讒・)
    amount_y = height - 85*mm
    pdf.setFont(font_name, 10)
    pdf.drawString(30*mm, amount_y, "荳玖ｨ倥・騾壹ｊ鬆伜庶縺・◆縺励∪縺励◆")
    amount_y -= 15*mm
