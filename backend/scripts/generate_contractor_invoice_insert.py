#!/usr/bin/env python3
"""
Generate SQL INSERT statements for contractor_invoices and contractor_invoice_details.
...
INVOICES_INPUT = os.path.join(SQL_DIR, 'contractor_invoices_input.csv')
DETAILS_INPUT = os.path.join(SQL_DIR, 'contractor_invoice_details_input.csv')
OUTPUT_FILE = os.path.join(SQL_DIR, 'contractor_invoice_insert.sql')
...
def generate_invoices_insert():
    """Generate INSERT statements for contractor_invoices"""
    statements = []
    
    with open(INVOICES_INPUT, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            invoice_id = clean_value(row.get('contractor_invoice_id'))
            ...
            # Required fields
            cols.append('contractor_invoice_id')
            vals.append(f"'{invoice_id}'::uuid")
            
            # Foreign keys
            if row.get('contractor_id'):
                cols.append('contractor_id')
                vals.append(format_sql_value(row['contractor_id'], 'number'))
            ...
            cols.append('total_discount_amount')
            vals.append(format_sql_value(row.get('total_discount_amount', '0'), 'number'))
            
            cols.append('total_after_discount')
            vals.append(format_sql_value(row.get('total_after_discount', '0'), 'number'))
            ...
            vals.append(format_sql_value(row.get('invoice_date'), 'date'))
            
            # Always include receipt_date even if empty
            cols.append('receipt_date')
            vals.append(format_sql_value(row.get('receipt_date', ''), 'date'))
            
            # Always include payment_due_date even if empty
            cols.append('payment_due_date')
            vals.append(format_sql_value(row.get('payment_due_date', ''), 'date'))
            ...
            stmt = f"INSERT INTO contractor_invoices ({', '.join(cols)}) VALUES ({', '.join(vals)}) ON CONFLICT (contractor_invoice_id) DO NOTHING;"
            statements.append(stmt)
    
    return statements

def generate_details_insert():
    """Generate INSERT statements for contractor_invoice_details"""
    statements = []
    
    with open(DETAILS_INPUT, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            detail_id = clean_value(row.get('contractor_invoice_detail_id'))
            invoice_id = clean_value(row.get('contractor_invoice_id'))
            ...
            # Required fields
            cols.append('contractor_invoice_detail_id')
            ...
            # Generate INSERT statement
            stmt = f"INSERT INTO contractor_invoice_details ({', '.join(cols)}) VALUES ({', '.join(vals)}) ON CONFLICT (contractor_invoice_detail_id) DO NOTHING;"
            statements.append(stmt)
...
def main():
    """Main execution"""
    print("Generating SQL INSERT statements for contractor invoices...")
    
    # Generate statements
    invoices_statements = generate_invoices_insert()
    details_statements = generate_details_insert()
    
    print(f"Generated {len(invoices_statements)} contractor_invoices INSERT statements")
    print(f"Generated {len(details_statements)} contractor_invoice_details INSERT statements")
    
    # Write to file
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
        f.write("-- Generated SQL INSERT statements for contractor_invoices and contractor_invoice_details\n")
        f.write(f"-- Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        
        f.write("BEGIN;\n\n")
        
        f.write("-- contractor_invoices\n")
    ...
    print("You can execute this file with: psql -f backend/sql/contractor_invoice_insert.sql")
