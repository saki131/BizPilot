...
#### 3.8.1 月別売上合計表示
- 年を選択すると、その年の月ごとの売上合計を一覧表示する
- 集計対象: 削除済みでない全販売員請求書 + 全委託先請求書
- 集計金額: 税込合計金額（`total_amount_inc_tax`）
- 表示項目:
  - 年月（YYYY年MM月）
  - 販売員請求書合計（税込）
  - 委託先請求書合計（税込）
  - 合計金額（税込・両者の和）
- 集計基準日: `invoice_date`（請求書の請求日）

#### 3.8.2 月別商品数量合計表示
- 月を選択すると、その月の商品ごとの数量合計を一覧表示する
- 集計対象: 選択月の販売員請求書明細 + 委託先請求書明細（親請求書が削除済みでないもの）
- 表示項目:
  - 商品名
  - 販売員請求書合計数量
  - 委託先請求書合計数量
...
#### 3.9.2 画像一括登録
- 注文台帳の画像をアップロード
- Gemini APIで画像から注文情報（顧客名、金額、メモ等）を読み取り
- 読み取り結果を確認・修正後、一括で注文登録
- 登録日は当日日付、入金期限は登録日+10日を自動設定

#### 3.9.3 注文一覧
- 全注文の一覧表示
- 「登録日」列で表示（旧「注文日」）
- 入金ステータスによるフィルタ（未入金・入金済）
- 顧客、期間によるフィルタ
- 入金期限超過かつ未入金の注文行は赤色で強調表示

#### 3.9.4 ステータス管理
- 未入金（unpaid）: デフォルト
- 入金済（paid）: 入金CSV照合完了時に自動更新、紐づく入出金明細IDを登録
- 入金期限超過かつ未入金の行は赤色表示（ステータスとしては「unpaid」のまま）

### 3.10 ゆうちょ入金履歴機能

#### 3.10.1 CSVアップロード
- ゆうちょダイレクトからダウンロードした入出金明細CSV（残高等通知明細）をアップロード
- CSVフォーマット: 取引日,入出金取引番号,お預り金額(円),お支払金額(円),詳細1,詳細2,残高(残付)額
- 入金行（お預り金額 > 0）のみを処理対象
- 重複アップロード防止機能

#### 3.10.2 自動照合（CSV取り込み時に実行）
- 照合条件（すべて一致で自動更新）:
  - 入金額 = 注文金額
  - 振込人名と顧客名の照合（完全一致または部分一致）
  - 入金日が登録日から入金期限の範囲内
- 完全一致の場合: 自動的に注文ステータスを「入金済」に更新し、入出金明細IDを紐付け
- 部分一致の場合（漢字↔カタカナ等）: フロントエンドで確認ダイアログを表示し、ユーザーがOKボタンを押した場合にステータスを更新

#### 3.10.3 入金記録一覧
- CSVから取り込んだ入金記録のみを表示（照合結果は表示しない）
- 入金日、入出金取引番号、振込人名、金額を表示
...
- 自動バックアップ: 毎日1回（02:00 JST）
- バックアップ保存期間: Google Drive 30世代・ローカル 7世代
- バックアップ方式: pg_dump カスタム形式（.dump）
- バックアップ実行基盤: GitHub Actions（PC不要・クラウド実行）
- バックアップ保存先: ローカル（`backend/data/backups/`）および Google Drive
...
### 6.5 売上統計API
```
GET    /api/sales-stats/monthly-totals            月別売上合計（年指定）
...
POST   /api/customer-orders/bulk                                    注文一括作成
POST   /api/customer-orders/recognize-image                         注文台帳画像認識
PUT    /api/customer-orders/{id}                                    注文更新
DELETE /api/customer-orders/{id}                                    注文削除
POST   /api/customer-orders/deposits/upload                         CSVアップロード（自動照合結果を返却）
